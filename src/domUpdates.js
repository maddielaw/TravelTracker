  ...
  updateTravelerTrips: function (trip, selector) {
    selector.innerHTML += `
      <div class="all-trips trip-card">
        ...
        <h4 class="num-travelers trip-info" id=${trip.id}>Travelers: ${trip.travelers}</h4>
        <h4 class="departure trip-info" id=${trip.id}>Departure Date: ${formatDate(trip.date)}</h4>
        <h4 class="duration trip-info" id=${trip.id}>Trip Duration: ${trip.duration} days</h4>
        <h4 class="trip-status trip-info" id=${trip.id}>Trip Status: ${trip.status}</h4>
      </div>`
  },
